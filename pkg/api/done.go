...
func doneHandler(w http.ResponseWriter, r *http.Request) {
	...
	if task.Repeat == "" {
		if err = db.DeleteTask(id); err != nil {
			writeError(w, "ошибка удаления задачи: "+err.Error())
			return
		}
	...
	if err != nil {
		writeError(w, "ошибка вычисления следующей даты: "+err.Error())
		return
	}

	if err = db.UpdateDate(id, next); err != nil {
		writeError(w, "ошибка обновления даты задачи: "+err.Error())
		return
	}
